(function () {
    ...
    const rightClickModeContainer = document.querySelector("#change-mode-container");
    const rightClickModeCheck = document.querySelector("#right-click-mode");

    const isMobile = (function () {
        // Original source: http://detectmobilebrowsers.com/
    ...
    button.addEventListener("mousedown", function (event) {
        if (isInvalidClick(event.button)) return;
        if (clickTypeIsCorrect(event.button)) {
            // CSS ':active' doesn't detects right click
    ...
    async function main() {
        ...
        if (isMobile) rightClickModeContainer.style.display = "none";
